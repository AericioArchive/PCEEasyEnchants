...
namespace Aericio\PCEEasyEnchants\tasks;

use Aericio\PCEEasyEnchants\PCEEasyEnchants;
...
class CheckUpdatesTask extends AsyncTask
{
    ...
    {
        $releases = Internet::getURL("https://poggit.pmmp.io/releases.json?name=PCEEasyEnchants");
    ...
    public function onCompletion(Server $server): void
    {
        if ($this->getResult() !== null) {
            $plugin = $server->getPluginManager()->getPlugin("PCEEasyEnchants");
            if ($plugin instanceof PCEEasyEnchants && $plugin->isEnabled()) {
                $data = json_decode($this->getResult(), true);
                $plugin->getLogger()->info("PCEEasyEnchants v" . $data[0]["version"] . " is available for download at " . $data[0]["artifact_url"] . "/PCEEasyEnchants.phar");
            }
        }
    }
